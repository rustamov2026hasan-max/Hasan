...
Currently configured project: `chorva-kuzatuv` (target: `..`).
...
original init command. Checkpoints in `data/chorva-kuzatuv/setup/setup-state.json`
...
`--project-id chorva-kuzatuv` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/chorva-kuzatuv/`.
